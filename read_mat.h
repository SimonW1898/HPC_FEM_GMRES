...
int readASCIIMatrix(char *filename, int *nr, int *nt, int **iat, int **ja, double **coef)
{
	int *nc = nr;
  int ierr = 0;
  FILE *fid;
...
int readCSRmat(char *fname, int *nrows, int *ncols, int *nterm, int **ia, int **ja, double **coef,
               bool BINREAD) {
   ...
   if (BINREAD) {
       ...
       ierr = readASCIIMatrix(fname,nrows,nterm,ia,ja,coef);
       if (ierr != 0) return 1;
   }
